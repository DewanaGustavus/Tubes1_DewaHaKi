...
public class Strategy {
    ...
    private static double xWalk(int degree){
        double rad=Math.toRadians(degree);
        double cosval=Math.cos(rad);
        if(bot.getSize()<50) return bot.getPosition().x + (10)*cosval;
        return bot.getPosition().x + (75)*cosval;
    }

    private static double yWalk(int degree){
        double rad=Math.toRadians(degree);
        double sinval=Math.cos(rad);
        if(bot.getSize()<50) return bot.getPosition().y + (10)*sinval;
        return bot.getPosition().y + (75)*sinval;
    }
    ...
    private static double getDistanceBetween(Position object1, double xx, double yy) {
        double dX = Math.abs(object1.x - xx);
        double dY = Math.abs(object1.y - yy);
        return Math.sqrt(dX * dX + dY * dY);
    }
    ...
    public static void moveLogic(PlayerAction playerAction){
        ...
        int bestDir=random.nextInt(360);

        for(int i=0;i<360;i+=10){
            double xafter=xWalk(i);
            double yafter=yWalk(i);
            ...
            for(int j = 2; j <=10; j++){
                ...
                else if(j==4) baseval=-70;
                else if(j==5) baseval=-10;
                else if(j==6) baseval=-10;
                else if(j==7) baseval=100;
                else if(j==8) baseval=10;
                else if(j==9) continue;
                else if(j==10) baseval=-10;
                if(objectList[j]!=null){
                for(GameObject obj : objectList[j]){
                    double curdist=getDistanceBetween(obj.getPosition(), xafter,yafter)-bot.getSize()-obj.getSize();
                    if(curdist>0){
                        curscore += baseval/(curdist*curdist);
                    }
                    else{
                        curscore += baseval*1000;
                    }
            ...
            for(GameObject obj:weakEnemy){
                ...
                double curdist=getDistanceBetween(obj.getPosition(), xafter,yafter)-bot.getSize()-obj.getSize();
                    if(curdist>0){
                        ...
                        curscore += baseval*100000;
                    }
            }
            for(GameObject obj:strongEnemy){
                double baseval=-2500;
                double curdist=getDistanceBetween(obj.getPosition(), xafter,yafter)-bot.getSize()-obj.getSize();
                    if(curdist>0){
                        curscore += baseval/(curdist*curdist);
            ...
            if (curscore>bestScore) {bestDir=i;bestScore=curscore;}
            
        }
        playerAction.heading=bestDir;
